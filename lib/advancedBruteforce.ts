...
export async function recordFailedAttemptAdvanced(request: NextRequest): Promise<BruteforceResult> {
  ...
  // Bloquer après 3 tentatives (30 secondes en dev, 5 minutes en prod)
  if (updatedAttempt.attempts >= 3) {
    const blockDuration = process.env.NODE_ENV === 'production' ? 5 * 60 * 1000 : 30 * 1000 // 5 min en prod, 30s en dev
...
async function sendBruteforceAlertAdvanced(attempt: {
  ip: string;
  fingerprint: string;
  browser?: string | null;
  os?: string | null;
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border-radius: 10px;">
        ...
        <div style="background: rgba(255, 255, 255, 0.1); padding: 20px; border-radius: 8px; margin-bottom: 20px;">
          ...
          <ul style="margin: 0; padding-left: 20px;">
            <li style="margin-bottom: 8px;">✅ Fingerprint et IP bloqués pendant ${process.env.NODE_ENV === 'production' ? '5 minutes' : '30 secondes'}</li>
        ...
        <div style="text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid rgba(255, 255, 255, 0.2);">
          <p style="margin: 0; opacity: 0.8; font-size: 14px;">
            Cet email a été généré automatiquement par le système de sécurité avancé d'Itineria.
          </p>
        </div>
      </div>
    `
