...
export default function ConfidentialitePage() {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setMessage({ type: 'success', text: t('accountDeletedSuccess') });
        ...
        const data = await response.json();
        setMessage({ type: 'error', text: data.error || t('accountDeleteError') });
        setTimeout(() => setMessage(null), 3000);
      }
    } catch (error) {
      console.error('Erreur lors de la demande de suppression:', error)
      setMessage({ type: 'error', text: t('accountDeleteError') });
      setTimeout(() => setMessage(null), 3000);
    }
  };
  ...
  const handleExportData = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json()
        setMessage({ type: 'error', text: data.error || t('exportError') })
      }
      setTimeout(() => setMessage(null), 3000)
    } catch (error) {
  ...
  return (
    <>
      ...
      <div className="space-y-6">
        {/* Export des données */}
        <div>
          <h3 className="text-lg font-semibold text-gray-700 mb-4">{t('dataExport')}</h3>
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="font-semibold text-blue-800 mb-1">{t('dataPortabilityRight')}</div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
              <div>
                <div className="font-semibold text-gray-700">{t('allMyData')}</div>
                <div className="text-sm text-gray-500">{t('allMyDataDesc')}</div>
              </div>
              <button 
        ...
        <div>
          <h3 className="text-lg font-semibold text-gray-700 mb-4">{t('deleteAccount')}</h3>
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="font-semibold text-red-800 mb-1">{t('irreversibleAction')}</div>
        ...
        <div>
          <h3 className="text-lg font-semibold text-gray-700 mb-4">{t('privacySettings')}</h3>
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
              <div>
                <div className="font-semibold text-gray-700">{t('dataSharing')}</div>
                <div className="text-sm text-gray-500">{t('dataSharingDesc')}</div>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-red-500 focus:border-transparent text-black"
                  placeholder={t('whyDeleteAccount')}
                />
              </div>
